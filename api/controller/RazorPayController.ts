...
if (!process.env.RAZORPAY_ID_KEY || !process.env.RAZORPAY_SECRET_KEY) {
  throw new Error(
    "Razorpay key_id and key_secret must be set in the environment variables."
  );
}
...
export const CreateOrder = async (req: Request, res: Response) => {
  try {
    ...
    const { amount, studentName, studentClass } =
      req.body as CreateOrderRequest;

    // Generate receipt
    const receipt = `Receipt #${crypto.randomBytes(10).toString("hex")}`;
    ...
    const generatingOrderId = await razor.orders.create(options);

    const {
      id,
      amount: orderAmount,
      receipt: orderReceipt,
    }: RazorpayOrderResponse = generatingOrderId;
...
export const FetchPaidPayments = async (req: Request, res: Response) => {
  try {
    ...
    const paidPayments = payments.items.filter(
...
export const CapturePayment = async (req: Request, res: Response) => {
  ...
  try {
    const captureResponse = await razor.payments.capture(
      paymentId,
      amount,
      currency
    );
...
// To verify Order when order Id is generated
  ...
  try {
    ...
    if (isAuthentic) {
      ...
      if (!classRecord) {
        return res
          .status(400)
          .json({ status: "failure", message: "Invalid class reference" });
      }
      ...
      res.status(422).json({ status: "failure", message: "Payment Failed" });
    }
  } catch (err) {
